...
def _import_file(module_name, file_path, make_importable=False):
    raise RuntimeError(
        "Code for drawing masks with holes was removed due to copyright. "
        "Please refer to the original Detectron2 repository for implementation."
    )
...
def fixup_module_metadata(module_name, namespace, keys=None):
    ...
    when they are referenced in docs, sphinx can find them. Reference trio
    """
    if not DOC_BUILDING:
        return
    seen_ids = set()
