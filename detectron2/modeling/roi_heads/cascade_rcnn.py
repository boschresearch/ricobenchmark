...
class CascadeROIHeads(StandardROIHeads):
    ...
    def forward(self, images, features, proposals, targets=None, keep_all_before_merge=False, batched_ignore_class_ids=None):
        del images
        ...
        if self.training or self.ewc:
            ...
            losses, scores_all, boxes = self._forward_box(features=features,
                                                   proposals=proposals,
                                                   targets=targets,
                                                   batched_ignore_class_ids=batched_ignore_class_ids)
            losses.update(self._forward_mask(features=features,
                                             instances=proposals,
                                             batched_ignore_class_ids=batched_ignore_class_ids))
            losses.update(self._forward_keypoint(features=features,
                                                 instances=proposals,
                                                 batched_ignore_class_ids=batched_ignore_class_ids))
            return proposals, losses, scores_all, sampled_idxs, boxes
        else:
            pred_instances = self._forward_box(features, proposals, keep_all_before_merge=keep_all_before_merge, batched_ignore_class_ids=batched_ignore_class_ids)
    ...
    def _forward_box(self, features, proposals, targets=None, keep_all_before_merge=False, batched_ignore_class_ids=None):
        """
        Args:
        ...
        if self.training or self.ewc:
            ...
            losses = {}
            scores_all = [predictions[0] for _, predictions, _ in head_outputs]
            storage = get_event_storage()
            for stage, (predictor, predictions, proposals) in enumerate(head_outputs):
                with storage.name_scope("stage{}".format(stage)):
                    stage_losses = predictor.losses(predictions, proposals, batched_ignore_class_ids=batched_ignore_class_ids)
                losses.update({k + "_stage{}".format(stage): v for k, v in stage_losses.items()})
            return losses, scores_all, boxes
        else:
