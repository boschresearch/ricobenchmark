...
# Copyright (c) CenterNet2 contributors.
import logging
import warnings
...
def fast_rcnn_inference(
    ...
    Args:
        ...
        topk_per_image (int): The number of top scoring detections to return. Set < 0 to return
            all detections.
        batched_ignore_class_ids (list[int]): a list of lists of class ids to be ignored during nms
    ...
    """
    if batched_ignore_class_ids is None:
        batched_ignore_class_ids = [None] * len(boxes)
    result_per_image = [
        fast_rcnn_inference_single_image(
            ...
            keep_all=keep_all_before_merge, ignore_class_ids=ignore_class_ids,
        )
        for scores_per_image, boxes_per_image, image_shape, ignore_class_ids in zip(scores, boxes, image_shapes, batched_ignore_class_ids)
    ]
...
def fast_rcnn_inference_single_image(
    ...
    if keep_all:
        ...
        if ignore_class_ids is not None:
            keep_mask = ~torch.tensor([c in ignore_class_ids for c in pred_classes], dtype=torch.bool)
            boxes = boxes[keep_mask]
            scores = scores[keep_mask]
            filter_inds = filter_inds[keep_mask]
            pred_classes = filter_inds[:, 1]
    ...
    if use_soft_nms:
        if not class_wise:
            ...
            try:
                ...
                warnings.warn("setting max_coordinate to 0." + str(boxes))
                max_coordinate = 0
            idxs = filter_inds[:, 1]
            offsets = idxs.to(boxes) * (max_coordinate + torch.tensor(1).to(boxes))
            boxes_for_nms = boxes + offsets[:, None]
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals, batched_ignore_class_ids=None):
        """
        Args:
        ...
        if batched_ignore_class_ids is not None:
            counts = [proposal.gt_classes.shape[0] for proposal in proposals]
            limits = [0] + list(itertools.accumulate(counts))
            predicted_cls = scores.argmax(dim=1)
            predicted_cls_sep = torch.split(predicted_cls, counts)
            keep_mask = torch.ones_like(predicted_cls, dtype=torch.bool)
            for i in range(len(predicted_cls_sep)):
                keep_mask_i = ~torch.tensor([c in batched_ignore_class_ids[i] for c in predicted_cls_sep[i]], dtype=torch.bool)
                keep_mask[limits[i]:limits[i+1]] = keep_mask_i
            scores = scores[keep_mask]
    ...
    # Implementation from CenterNet2 with slight modifications
    ...
    # Implementation from CenterNet2 
