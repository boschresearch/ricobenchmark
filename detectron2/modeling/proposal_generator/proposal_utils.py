...
def find_top_rpn_proposals(
    ...
    for level_id, (proposals_i, logits_i) in enumerate(zip(proposals, pred_objectness_logits)):
        ...
        topk_indices.append(topk_idx + start_idx)
        level_ids.append(
            move_device_like(
    ...
    for n, image_size in enumerate(image_sizes):
        ...
        indices_per_img = topk_indices[n]
        lvl = level_ids
        ...
        if not valid_mask.all():
            ...
            lvl = lvl[valid_mask]
            indices_per_img = indices_per_img[valid_mask]
        ...
        if _is_tracing() or keep.sum().item() != len(boxes):
            ...
            indices_per_img = indices_per_img[keep]
            lvl = lvl[keep]
        ...
        res.indices = indices_per_img[keep]
        results.append(res)
    return results
...
def add_ground_truth_to_proposals_single_image(
    ...
    if proposals.has('indices'):
        gt_proposal.indices = torch.arange(-len(gt_logits), 0, 1, device=device).flip(dims=(0,))
