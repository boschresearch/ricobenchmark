...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.freeze_backbone:
            self.perform_freeze_backbone()

        
        
    def perform_freeze_backbone(self):
        ...
        unfreeze_lora = True
        unfreeze_prompt = True
        
        if unfreeze_lora or unfreeze_prompt:
            for p in self.backbone.named_parameters():
                if 'lora' in p[0] or 'prompt' in p[0]:
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], task_id=None, cfg=None):
        """
        Args:
        ...
        """
        assert cfg is not None, "cfg is required for training"
        
        if not self.training and not self.ewc:
            return self.inference(batched_inputs)

        batched_ignore_class_ids = []
        for inputs in batched_inputs:
            if 'ignore_class_ids' in inputs:
                batched_ignore_class_ids.append(inputs['ignore_class_ids'])
            else:
                batched_ignore_class_ids.append([])
        ...
        features = self.backbone(images.tensor, task_id)
        if 'diverse' in features:
            # for the 'reduce_sim' loss
            diverse = features['diverse']
            del features['diverse']
        else:
            diverse = None
        ...
        with torch.cuda.amp.autocast(enabled=False):
            if self.proposal_generator is not None:
                proposals, proposal_losses, rpn_data = self.proposal_generator(images, features, gt_instances)
            else:
            ...
            proposals_roi_heads, detector_losses, scores, sampled_idxs, boxes_head = self.roi_heads(images, 
                                                                                        features,
                                                                                        proposals,
                                                                                        gt_instances,
                                                                                        batched_ignore_class_ids=batched_ignore_class_ids)
        ...
        proposal_losses = self.proposal_generator.losses(**rpn_data,
                                                         scores=scores,
                                                         batched_ignore_class_ids=batched_ignore_class_ids,
                                                         sampled_idxs=sampled_idxs,
                                                         proposals_roi_heads=proposals_roi_heads,
                                                         ignore_boxes_threshold=cfg.train.ignore_boxes_threshold_rpn,
                                                         use_ignore_mask=cfg.train.use_ignore_mask_rpn,
                                                         reduce_ignore=cfg.train.reduce_ignore,
        ...
        if diverse is not None:
            losses.update({'reduce_sim': -0.1*diverse['reduce_sim']})
        return losses
    ...
    def preprocess_image(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        means = [
            self._move_to_current_device(
                torch.tensor(b['mean']).view(-1, 1, 1)
            )
            for b in batched_inputs
        ]
        std = [
            self._move_to_current_device(
                torch.tensor(b['std']).view(-1, 1, 1)
            )
            for b in batched_inputs
        ]
        images = [(images[i] - means[i]) / std[i] for i in range(len(images))]
