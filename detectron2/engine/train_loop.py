...
class AMPTrainer(SimpleTrainer):
    ...
    def __init__(self, model, data_loader, optimizer, grad_scaler=None, cfg=None):
        ...
        # self.use_ewc = cfg.train.use_ewc
        # if self.use_ewc:
        #     self.ewc = EWC(save_dir=cfg.train.output_dir, freeze_backbone=model.freeze_backbone)
        #     self.ewc.load_fisher_matrix(cfg.train.ewc_path)
    ...
    def run_step(self):
        ...
        while True:
            data = next(self._data_loader_iter)
            data = [d for d in data if len(d['instances'].gt_boxes) > 0]
            if len(data) > 0:
                break
        data_time = time.perf_counter() - start

        with autocast():
            loss_dict = self.model(data, cfg=self.cfg)
            if isinstance(loss_dict, torch.Tensor):
                losses = loss_dict
                loss_dict = {"total_loss": loss_dict}
            else:
