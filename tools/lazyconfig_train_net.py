...
def do_test(cfg, model, eval_only=False):
    ...
    if eval_only:
        ...
        if "evaluator" in cfg.dataloader:
            ret = inference_on_dataset(
                cfg.train.max_iter_test, model, instantiate(cfg.dataloader.test), instantiate(cfg.dataloader.evaluator)
            )
            print_csv_format(ret)
        return ret
...
def do_train(args, cfg):
    ...
    trainer = (AMPTrainer if cfg.train.amp.enabled else SimpleTrainer)(model, train_loader, optim, cfg=cfg)
    ...
    trainer.register_hooks(
        [
            ...
            hooks.BestCheckpointer(eval_period=cfg.train.eval_period, checkpointer=checkpointer, val_metric="bbox/AP", mode="max"),
...
# def do_calc_fisher_matrix(args, cfg):
#     """"""
#     model = instantiate(cfg.model)
#     model.backbone.ewc = True
#     model.proposal_generator.ewc = True
#     model.roi_heads.ewc = True
#     model.to(cfg.train.device)
#     model = create_ddp_model(model)
    
#     trainer = SimpleTrainer(model, None, None, cfg=cfg)
...
#     train_loader = instantiate(cfg.dataloader.train)
#     ewc = EWC(save_dir=cfg.train.output_dir, freeze_backbone=cfg.model.freeze_backbone)
#     ewc.compute_fisher_matrix(model, train_loader, cfg.train.max_iter, grad_scaler=None)
#     ewc.save_fisher_matrix()
#     sys.exit()
...
def main(args):
    ...
    if args.save_fisher_matrix:
        do_calc_fisher_matrix(args, cfg)
