...
inclement_data = load_data_from_json('/datasets/dense/annotations_inclement.json')
inclement_paths = [x['file_name'] for x in inclement_data]
inclement_paths = [x.replace('cam_stereo_left_lut', 'gated_full_acc_rect8') for x in inclement_paths]
...
for i, file in enumerate(os.listdir(label_folder)):
    ...
    if file_name in inclement_paths:
        continue
    object_i = {
        'file_name': file_name,
        'width': 1280,
    ...
    for label in labels:
        if label['identity'] in images_to_remove_by_class:
            save_img = False
            break
        label_counter[label['identity']] += 1
...
for i, file in enumerate(os.listdir(label_folder)):
    ...
    for label in labels:
        if label['identity'] in images_to_remove_by_class:
            save_img = False
            break
        if label['identity'] not in classes_rename_ccl:
            continue
        area = (label['xright'] - label['xleft']) * (label['ybottom'] - label['ytop'])
        if area < 49:
            continue
        object_i['annotations'].append({
            'category_id': classes_rename_ccl[label['identity']]['id'],
            'category_name': classes_rename_ccl[label['identity']]['name'],
            'bbox': [label['xleft'], label['ytop'], label['xright'], label['ybottom']],
            'bbox_mode': 0,
        })

    if save_img and len(object_i['annotations']) > 0:
        data_ccl.append(object_i)
...
for i in range(len(data_ccl)):
    date = os.path.basename(data_ccl[i]['file_name']).split('_')[0]
    data_ccl[i]['scene_id'] = scenes[date]


save_data_as_json(data, '/datasets/dense/annotations_gated.json')
for i in range(len(data_ccl)):
    data_ccl[i]['ignore_class_ids'] = [2, 3, 4, 5, 6, 7]
save_data_as_json(data_ccl, '/datasets/dense/annotations_gated_ccl.json')
