...
annotation_file = "/datasets/synscapes/bboxs.json"
annotation_file = load_data_from_json(annotation_file)
...
file = '/datasets/synscapes/Synscapes/meta/20216.json'
d = load_data_from_json(file)


data = load_data_from_json("/datasets/synscapes/bboxs.json")
...
data_day = [data[i] for i in range(len(data)) if means[i] > 0.38]

for i in range(len(data_day)):
    data_day[i]['scene_id'] = i

save_data_as_json(data_day, "/datasets/synscapes/annotations_day.json")
